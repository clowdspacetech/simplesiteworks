          ...
          {TRUST_STATS.map((stat, index) => (
            <Reveal key={stat.label} delay={(index % 4) as 0 | 1 | 2 | 3}>
              <div className="ssw-card h-full text-center sm:text-left">
                <div
                  className={`font-display font-extrabold tracking-tight text-transparent bg-gradient-to-r from-indigo-300 via-cyan-300 to-violet-300 bg-clip-text md:whitespace-nowrap ${
                    stat.value.length > 8 ? "text-3xl sm:text-4xl" : "text-4xl sm:text-5xl"
                  }`}
                >
                  {stat.value === "5+ Hours Saved Weekly" ? (
                    <>
                      5+ Hours <span className="block sm:inline">Saved Weekly</span>
                    </>
                  ) : (
                    stat.value
                  )}
                </div>
                <h3 className="mt-3 text-sm font-extrabold tracking-tight text-white md:whitespace-nowrap">
                  {stat.label}
                </h3>
