        ...
        <div className="ssw-grid grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {TRUST_STATS.map((stat, index) => (
            <Reveal key={stat.label} delay={(index % 4) as 0 | 1 | 2 | 3}>
              <div className="ssw-card flex h-full flex-col text-center sm:text-left">
                <p className="font-display text-4xl font-extrabold tracking-tight whitespace-nowrap text-transparent bg-gradient-to-r from-indigo-300 via-cyan-300 to-violet-300 bg-clip-text sm:text-5xl">
                  {stat.value}
                </p>
                <h3 className="mt-3 text-sm font-extrabold tracking-tight text-white">
                  {stat.label}
                </h3>
